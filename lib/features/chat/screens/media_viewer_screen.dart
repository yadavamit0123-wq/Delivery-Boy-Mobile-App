...
class _MediaViewerScreenState extends State<MediaViewerScreen> {
    ...
    if(_fromNetwork && (widget.serverMedia?.isNotEmpty ?? false)){
      if(chatController.isVideoExtension(widget.serverMedia?[currentIndex].path ?? '')){
        controller =  VideoPlayerController.networkUrl(
          Uri.parse('${widget.serverMedia![currentIndex].path}'),
        );
        ...
        errorBuilder: (context, error) =>  Center(child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('this_video_is_not_supported_please_download_and_pay'.tr , style: rubikRegular.copyWith(
  ...
  Future<void> _download(String? url) async {
    ...
    if(status.isGranted){
      ...
      Get.find<ChatController>().downloadFile(url, directory!.path, "${directory.path}/$url", url);
